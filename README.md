...
- 置顶悬浮小窗口，常驻系统托盘（不占任务栏）
- 托盘左键点击切换窗口显示/隐藏，右键菜单退出
- 支持拖动，靠近屏幕边缘自动吸附
- 横向 / 竖向布局切换（右键菜单）
- 开机自启（右键菜单开关）
- HTTP API 驱动，无 Hooks 依赖
- Tauri 原生壳，内存占用约 35MB
- 便携版 exe（约 18MB），无需安装
...
- 直接运行 `dist/Light-for-all-Agent-0.1.0-x64-portable.exe`（需 `WebView2Loader.dll` 同目录），即可在桌面看到悬浮状态灯
- 应用常驻系统托盘（右下角），任务栏不显示图标
- 托盘左键点击可隐藏/显示悬浮窗；悬浮窗右键菜单可切换横向/竖向、开机自启和退出
- 应用会实时展示状态变化，并通过 HTTP 接口与 Agent 联动

## 技术栈

- **前端**：HTML + CSS + JS（无框架，纯静态）
- **后端**：Rust + Tauri v2（GNU 工具链，锁定 Rust 1.94，含系统托盘）
- **通信**：HTTP REST API（`127.0.0.1:37421`）
- **包体**：约 18MB 便携版 exe（静态链接 libgcc，附 WebView2Loader.dll）
